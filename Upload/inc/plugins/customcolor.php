...
if(!defined('IN_ADMINCP'))
{
    ...
    $templatelist .= 'customcolor_headerinclude, customcolor_footer, customcolor_input1';

    if(defined('THIS_SCRIPT') && THIS_SCRIPT == 'usercp.php')
    {
        $templatelist .= ', customcolor_usercp, customcolor_input2';
    }
}
...
.colorpicker_slider {
    ...
    // Setting 1 enable/disable input

    $setting_1 = array(
        'name'          => 'customcolor_enable_input',
        'title'         => $lang->customcolor_enable_input_title,
        'description'   => $lang->customcolor_enable_input_desc,
        'optionscode'   => 'yesno', 
        'value'         => '0', 
        'disporder'     => '1', 
        'gid'           => intval($gid)
...
$(document).ready(function($){
    ...
    $insert_array_3 = array(
        'title' => 'customcolor_input1',
        'template' => $db->escape_string('<span class="custom_theme" title="{$lang->colorucp_input}"><input type="text" id="colorpicker" value="{$mybb->settings[\'customcolor_default\']}" /></span>'),
        'sid' => '-2',
        'version' => '',
        'dateline' => time()
    );
...
{ 
    ...
    $db->insert_query('templates', $insert_array_8);

    // Template _css_texts

    $insert_array_9 = array(
...
{ 
    ...
    $db->insert_query('templates', $insert_array_9);
    ...
    find_replace_templatesets('header_welcomeblock_member', '#'.preg_quote('{$buddylink}').'#i', '{$customcolor_input1}{$buddylink}');    
...
function customcolor_deactivate()
{
    ...
    find_replace_templatesets('header_welcomeblock_member', '#'.preg_quote('{$customcolor_input1}').'#i', '', 0);
}

function customcolor_headerinclude()
{
    global $mybb, $templates, $customcolor_headerinclude, $customcolor_footer, $customcolor_input1, $customcolor_input2;
    ...
    if ($mybb->settings['customcolor_enable_input'] == 1)
    {
    ...
    if($mybb->usergroup['canusercp'])
    {
        ...
        $customcolor_input1 = eval($templates->render('customcolor_input1'));
    ...
    if ($mybb->settings['customcolor_enable_input'] == 0)
    {
    ...
    if($mybb->usergroup['canusercp'])
    {
        ...
        $customcolor_input2 = eval($templates->render('customcolor_input2'));
...
{
	global $themes, $mybb, $templates, $theme, $customcolor, $lang, $customcolor_input2;
